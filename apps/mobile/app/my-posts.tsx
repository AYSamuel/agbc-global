...
export default function MyPostsScreen() {
  ...
  // Rows apart from placeholders, so the list can virtualize (W4.7 slice 3).
  // The four states are unchanged; only their shape moved.
    ...
    <ListScreen
      ...
      header={
        <>
          ...
          />
          {/* The frame opens straight onto the cards, with 8px of air and no
              explanatory line: the pills say what each post is doing, and a
              paragraph above them would be the screen explaining itself
              instead of answering. */}
          {rows.length > 0 ? <View style={{ height: spacing.xs }} /> : null}
        </>
      }
      ...
      // states bring their own padding, so this only insets the list.
