...
- **Web deletion path (Play Store requirement): BUILT W4.5 slice 4 (2026-09-01)**, as `/delete-account` on the church website (`Desktop/agbc`) plus the `web-delete-account` edge function here. Enter email → OTP → confirm, one form, two steps, and the same `erase_profile()` the app calls. Linked from the Play data-safety form, so the URL cannot be renamed without a store filing, and from the privacy policy's "Your rights" section in all four locales, which is the only route to it from the site itself. See `20`.
  - **The website deliberately does not delete anything**, though it holds the secret key and could. Its half is the form, the honeypot, Turnstile and the per-IP limit; the decision to erase stays in this repo, next to the routine that carries it out. Split the other way, the deletion rules would live in two repos on two release schedules, and the website would be one refactor away from erasing somebody without an OTP.
  - **`shouldCreateUser: false` is load-bearing on the send.** Without it, asking to delete an address that never signed up CREATES an account for it, so a form offering erasure would be a way to make accounts for other people's inboxes.
  - **No session ever leaves the function.** Verifying an OTP necessarily mints one; it is used only to learn which user id the code proved, then discarded, and it does not even need revoking because `erase_profile` deletes every session that user has inside the same transaction that answers for it.
  - **Two shapes of account reach the confirm and only one has a profile.** Somebody who signed in once and never finished AUTH-3 has an auth user and no profile row; `erase_profile` refuses that (`no_data_found`), and leaving it would occupy the address for ever against a promise that it can register again. That case deletes the auth user outright, which is safe precisely because nothing points at it.
  - **The last-admin refusal reaches the web caller as a generic outage message**, since `erase_profile` raises and every other answer here is uniform by design. Accepted: it can only ever be seen by one person, who has the app's own DELETE screen saying why.
