...
- **Sliced 2026-08-12 into three, with the native rebuild between 1 and 2** so both SDKs ride one EAS build rather than two:
  ...
  2. **Instrumentation.** Fire the 17 events whose surfaces exist (`EVENT_SOURCE` in `packages/shared/src/analytics/events.ts` is the register: 17 say `fires: 'app'`; an earlier revision of this line said 14, which was a miscount), at the mutation/hook layer rather than in components, then confirm the north-star events arriving in PostHog from a dev device. **Ready to start (2026-08-13):** the PostHog key is in EAS `preview`/`production` and in the local `.env`, so a dev device sends. Three placements are still owed and each fails silently, so the table in `docs/runbooks/credentials.md` §"Observability values" is the authority, not this line: `SENTRY_AUTH_TOKEN` (Ayo creates it; without it release builds keep minified traces for ever), the dashboard's DSN at its first Vercel deploy, and the edge DSN at the `19` cutover. Also note the fifth standard property `environment`, which exists only because PostHog's free plan allows one project (ADR 0020 amendment): every insight must filter on it or the wedge baseline includes our own testing.
