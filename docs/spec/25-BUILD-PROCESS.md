...
keeps a real calendar) is built and merged but not shipped: it goes out with the next store
build, and its two migrations must be dispatched to production with that build and never before
it (its board entry says how). With that, no Phase 4 item is open for building, and a session
...
**W4.5 · Account deletion + web deletion path · COMPLETE (2026-09-02)**
...
- **Landed in five slices**: the erasure itself in one transaction (`20260901140000`, `150000`, `160000`), the address and the file sweep (`20260901170000`, `erasure-sweep`), the DELETE screen and the deleted-account transition, the web path (`web-delete-account` here, `/delete-account` in `Desktop/agbc`), and the reach walk. Building slice 1 against the real schema turned up FIVE places where `16`'s reach could not run as written, all corrected there.
- **Six lessons, and the first two generalise furthest.**
  1. **A fixture with a shape the real writer never produces proves nothing.** Nulling `auth.users.email` does not free the address: `auth.identities.email` is a GENERATED column the signup check also reads, so a signup for the same address is still refused `422 email_exists`. Slice 1 shipped that bug green, because every seeded auth user in this project is inserted straight into `auth.users` and has no identity row at all. It was found by driving the real auth API, not by reading code.
  2. **An enumeration test is only as good as its fixture, and its own comment will tell you so if you read it.** `054` swept every foreign key into `profiles` and reported nothing left behind. Fourteen of the twenty-two roads had no row on them, so it was reporting that it had found nothing where nothing had been put. The fix is one list read twice: assert the fixture reaches every road BEFORE the erasure, sweep the same list after. Mutation-checked in the direction that matters, by unseeding one road and watching only the new guard go red.
  3. **The defects that reached a screen were copy and CSS, again** (the same finding as W4.0). A kept post credited to "A former member" told the whole congregation that people leave, which Ayo caught on the device; the confirm button kept its "Send me a code" label because the restore read the step the submit STARTED with; and `hidden` on a form does nothing when the stylesheet says `display: grid`, so the success message sat above a live delete button. None of the three is visible to a passing test suite and all three are obvious on screen.
  4. **A test can assert the environment by accident.** The web page's first e2e asserted the Turnstile widget was present, which passes only where a site key is configured; CI configures none. What holds everywhere is the pairing, that any widget on the page sits inside the form whose token it supplies.
  5. **The half that decides must be the half that owns the rules.** The website holds the secret key and could call `erase_profile` directly. It does not, because then the rules live in two repos on two release schedules, and the site is one refactor away from erasing somebody without an OTP.
  6. **THE LAST BUG WAS ONLY EVER FINDABLE IN PRODUCTION, and it was found there (2026-09-02).** Driving the live website against the live database, the confirm step returned `permission denied for function erase_profile`. The grant was correct: production confirms `service_role` holds EXECUTE. The CALLER was wrong. supabase-js stores the session `verifyOtp` mints and attaches that member's access token to every later request from the same client, so the erasure ran as `authenticated` and hit this item's own `revoke ... from authenticated`. The database refused exactly what it should refuse. No unit test could have caught it, because the fake would have to reproduce the library bookkeeping that was the unknown, and every layer passed on its own: 1657 database assertions, 175 Deno tests, 26 browser tests. **A green suite means the parts work; it never means the parts have met.** The end-to-end drive is not ceremony at the end of an item, it is the only test of the seams between the halves that ship separately.
  7. **`02` described a bucket that has never existed.** `avatars` is in the storage table and in no migration, harmless only because nothing writes `profiles.avatar_url` yet. Recorded there with what it costs the day an uploader is built.
...
**W4.9 · Audio shelf and player · COMPLETE (2026-09-06)**, five slices in five PRs (#253 the shelf, #254 seek and speed, #255 the now-playing bar, #256 the Watch segment, then the docs), every screen change framed in `entry-flow.html` or `dashboard.html` and approved by Ayo before code. The plan file is gone, per its own instruction; what it decided lives in `08` (the segment, seek and speed, listening surviving navigation), `05` (the knob, the listen glyph, the bar, the segment), `17` §4 and `02` §Storage (MP3 only under 50 MB, the edit form, no delete), `18` (the audio owner line), the migration `20260906120000` and the module comments of `features/watch/nowPlaying.ts`, `lockScreen.ts` and `segment.ts`.
- Why it existed: the first sermon audio ever put on production (2026-09-05, to film the Play declaration) and one evening of real use found six things a green suite had never rendered: the manage screen crashed for any message with audio, the dashboard promised 150 MB while the Free plan refuses 50, the picker took an m4a the spec never wanted, the player could neither seek nor pass 1.5x, leaving the player stopped the listening, and an audio-only message had no home in Watch.
- Lessons that generalise past this item:
  - **Copy that states a limit is a claim the system must be able to keep.** 150 MB was written in three places and true in none; a refused upload that says "check your connection" for a size cap is worse than no message. The cap is now one constant with the Free plan named beside it, and the refusal names the file, storage or the connection, whichever it was.
  - **A byte check is only as strict as the format it can tell apart.** ADTS AAC shares MP3's frame sync, so the "is this an MP3" check accepted an aac file until the Layer III bits were read; a check that looks at one byte is a check that matches its own assumption.
  - **Responder props lose to Android's scroll view.** A native scroll view intercepts a drifting vertical move within the first few events whatever the JS responder asks, and `scrollEnabled={false}` set from the grant lands a frame late; a gesture claimed natively (`react-native-gesture-handler`, already a dependency through Expo Router) is the only thing that holds a drag. Found by Ayo on the tablet after the unit matrix was green.
  - **A native ceiling is a product decision, not a bug to route around.** expo-audio clamps Android at 2x while iOS passes the rate through; 3x would have meant a patched native module and a control behaving differently per platform, so the ceiling is 2x and the reason sits beside the constant.
  - **The lock screen has one owner and the handoff is ours** (PR #252): two activations without a stand-down between them crash the app past every JS try/catch. Go through `activateLockScreen()` / `releaseLockScreen()`, key the release effect on the player alone, and update metadata in place. A fake that hands the same object back for a second player hides a missing re-activation, so the test double now mints a distinct object per player and the ordering spies are the proof.
  - **The tablet shell remounts the whole stack on a layout change**, which resets every screen's state and renders every screen in the history at once. Two defects grew from it: the sermon screen came back in video mode and stopped its own audio (its mode now seeds from the provider), and a message visited earlier stopped the one playing now (the video-stops-audio rule is a FOCUS effect, and the test mock of `useFocusEffect` gained a focus flag so an unfocused mount can be modelled). Any rule about "the screen the member is looking at" must be a focus effect, never a plain one.
  - **A finished native player does not move on `play()`**; "the end is start over" has to seek to 0 itself. Found on the tablet, invisible to a fake that plays from wherever it is told.
  - **A default column value is a classification.** Audio-only rows carry the sync's default `video` kind because they were never synced, and through it they leaked into the Video rail and took the hero; the split is now on the fact (`youtube_id`, `audio_path`), not the label.
  - **`uiautomator`'s accessibility tree goes stale on the tablet after navigation** until a rotation refreshes it, reporting a screen already left and none of the new one; touches were unaffected. Whether TalkBack sees the same is unproven and is owed to the tablet shell.
- **Owed items closed on 2026-09-07, on the tablet and then the S22.** The metadata refresh is proven on the device: with a message playing, its title was changed in the database, and once the feed went stale and the player was reopened from the bar the media session read the corrected title with the SAME session id (no second activation) and an empty crash buffer; the bar and Home's latest-message card followed. The phone pass ran on the S22 with the EAS dev client installed over the production build for the evening (the phone had been carrying versionCode 22, which cannot be downgraded over, so it was uninstalled and the preview build put back afterwards): the bar above the tab bar on Home, Family, More and Watch, at the bottom edge on Settings, both Watch halves with the listen glyphs, the audio list, and the cross emptying the session. **Still owed: TalkBack itself.** Injected input is not a faithful TalkBack instrument (a single tap activated the bar instead of only focusing it, swipes and the keyboard shortcut moved the focus outline nowhere, and the speech-output overlay's permission refuses adb's grant), so the spoken traversal of the bar and the segment is a hands-on check per `21` §4; the tree it reads (roles, labels, selected state) is verified. The stale accessibility tree after leaving the player was seen on the PHONE too, so it is not the tablet shell's: `uiautomator` reports the sermon transport after the screen is gone until a tab switch or rotation refreshes it, and touches are unaffected. The two hand-inserted audio-only rows on the local stack go with the next `pnpm db:reset`.

**W4.10 · Updating from inside the app · COMPLETE (2026-09-08)**, three slices in one PR, shipped as release **1.0.1 / build 23**. Two frames approved before code (`entry-flow.html` `W4.10`): UPDATE-NOTICE, and UPDATE-REQUIRED drawn at last as it actually ships, having been built at W1.2 with no frame at all. What it decided lives in `21` §7-9, `01` §1 and §6, `23` (b), `18`, `docs/runbooks/store-submission.md` §4b, the new `docs/runbooks/releases.md`, and the plan file `docs/spec/plans/W4.10-in-app-updates.md`.
- Why it existed: Ayo asked why an update still meant "go and find us in the store". The app had only the WALL, so the ONLY update a member ever heard about was one that had already locked them out, and even that screen linked out and left them to find the Update button themselves.
- **The decision underneath it is that there will be no OTA.** `21` had assumed EAS Update since it was written (§118 defines it, §122's cadence is "OTA between trains", §113's rollback starts with it, §133 budgets its meter) and none of it had ever run: `expo-updates` is not installed. The assumption is dropped rather than deferred, because the experience asked for is Play's in-app updates API and is not OTA at all. What the choice costs is now written down instead of being rediscovered mid-incident: no same-day fixes, rollback loses its fast half, and every fix spends one of 15+15 monthly builds.
- Lessons that generalise past this item:
  - **Half-laid groundwork reads as a working feature.** `runtimeVersion: { policy: 'fingerprint' }` was set and `eas.json` carried three update channels, so the repo looked like it had OTA; nothing served anything, and the giveaway was one absent dependency. Config that only makes sense with a runtime should say which runtime, or it is a claim the repo cannot keep.
  - **A generality nobody will use is not worth its footguns.** The plan recommended a release counter in the binary mirrored in `app_config`, and building it killed it: its whole advantage was reading the same on both platforms, and the escalation is Android-only because Apple has no takeover flow. What survived was a second config key, an integer every train has to remember, and a member locked into a Play install screen if anyone forgot it twice. Play's own `clientVersionStalenessDays` needs no state anywhere.
  - **When the consequence is taking somebody's screen, every arm of the decision should belong to somebody else.** The escalation cannot fire on our say-so: Play must report the update, allow the immediate flow AND supply the staleness, and a missing staleness declines rather than reading as zero. That is the forced-update gate's fail-open rule applied to a softer surface.
  - **A feature can be correct and unverifiable at the same time, and that has to be said out loud in three places.** Play's in-app updates work only on a build the Play Store itself installed, so a sideloaded EAS APK reports "no update available" forever and the first real test of this feature is the first release AFTER the one that ships it. It is recorded in `18`, in the plan and in the runbook, because a green suite here proves the wiring and nothing about the platform.
  - **An inert integration is safer than an absent one when the failure is a resolve, not a throw.** iOS reads `AppStoreID` from the Info.plist, the app has never had one, and the iTunes lookup then matches nothing and RESOLVES "no update available". Checking what a missing config does to the native call, rather than assuming it throws, is what made it safe to ship the iOS half switched off.
  - **A dirty working tree is shared state.** Two of this item's runbook edits were swept into someone else's docs commit (`9f004f4`, PR #262) minutes after they were written, because another session committed everything it found. Nothing was lost and the content is on `main`, but the PR that owns the work does not contain it, so the record had to be corrected by hand. Commit or stash before anything else in the repo starts moving.

**W4.11 · The three things Play told us about release 22 · COMPLETE (2026-09-08)**, in one PR, straight after `23 (1.0.1)` went into review. Read off the Production release dashboard in the console rather than from memory; all three re-attached themselves to 23 the moment it was submitted, so none was fixed by that release. The plan is `docs/spec/plans/W4.11-store-recommendations.md`; what it decided lives in `05` §Tablet, `21` §8, `lib/orientation.ts` and `app.config.js`.
- Why it existed: Ayo asked for all three to be fixed. One of them could not be, one was a real bug our own spec had contradicted for months, and one is a five-month deadline that no test can verify.
- **The orientation one was a bug with users behind it.** `orientation: 'portrait'` became `android:screenOrientation="PORTRAIT"` on MainActivity, so a tablet on **Android 15 or below was held in portrait and could never reach the layouts W4.7 built**. Android 16 ignores orientation locks on large screens, which is the only reason the tablet work was ever testable, so those layouts existed for exactly one Android version. The rule moved into the app (`lib/orientation.ts`) where it can be conditional: portrait on a phone, untouched on a tablet, player rotates anywhere.
- Lessons that generalise past this item:
  - **THE DEFAULT WINDOW UNDER JEST IS A TABLET.** jest-expo reports 750x1334, whose smallest side is 750 against `layout.ts`'s 600dp line, so anything conditional on `isTablet` is silently ON in every test that does not set a size. The existing landscape test had been exercising the tablet branch while reading as though it tested a phone, and only went red when the behaviour it asserted became conditional. Any test touching a device class must state the device.
  - **A tool's own advice is three different kinds of thing wearing one badge.** Of Play's three items, one is ours and urgent-ish, one is ours and was a real bug, and one is React Native's and Google Material's own code with no call site of ours anywhere. Reading the detail rather than the headline is what separated them; `21` §8 now carries the six library entry points so nobody investigates it twice.
  - **`expo prebuild` edits `package.json`**, rewriting the `android` and `ios` scripts to `expo run:*`. Harmless-looking, and on this machine `expo run:android` is the local Gradle path that cannot work (MAX_PATH against pnpm's store). Reverted by hand. A prebuild run to INSPECT output is worth doing and its side effects are worth checking afterwards.
  - **Ask the artefact, not the config.** Setting an option is not evidence it lands. A throwaway prebuild proved that the manifest now says `screenOrientation="unspecified"` (Android's no-restriction default, and NOT an absent attribute, which is worth knowing before assuming Play's panel clears) and that `android.enableMinifyInReleaseBuilds=true` reaches `gradle.properties` where `minifyEnabled` reads it, with `shrinkResources` left false.
  - **R8 is the one change in this repo that no layer can verify.** It removes classes it cannot see used, and React Native, Hermes and every Expo module find things by reflection; a stripped class fails at runtime on the screen that needed it while the build, typecheck, lint and all 1128 tests stay green. The acceptance test is an installed release artifact walked by hand, which is the same shape as W4.8's placeholder icon: the built artefact is the one surface no test looks at.
- **Owed, and it gates whichever release carries this:** the R8 walk on that installed build, before any production rollout. **The item is merged and unshipped by decision** (Ayo, 2026-09-08: not enough yet to justify a release), so both owed checks travel with the next store train rather than being due now. And the tablet pass belongs on a tablet running **Android 15 or below**, which is the only case slice 1 actually changes; on an Android 16 tablet the before and after are identical by construction.

**W4.12 · Dashboard hardening: navigation, loading states, feedback · COMPLETE (2026-09-11)**, eight PRs (#282 the data-loss fix, #283 the baseline, #284 pending states and contrast, #285 the content column, #286 the shared layout, #287 the frames, #288 the loading states, #289 the rail). Plan at `docs/spec/plans/W4.12-dashboard-hardening.md`; what it decided lives in **ADR 0025**, `17` §Navigation and loading, and the ten loading frames in `design/mockups/dashboard.html`.
- **The result, measured the same way before and after.** A rail click WAS a full document load that re-parsed **810 KB** of JavaScript from cache and showed nothing at all for ~259 ms. It is now a client transition that puts the destination's skeleton on screen in **5 ms median** (3 to 17 over seven hops) with the rail never unmounted. Server render time is unchanged and was never the thing to fix.
- **The order of the slices was itself a finding, and it changed mid-item.** The plan was `<Link>` first. Slice 1's measurement showed the server wait dominates (187 ms TTFB of a 259 ms load), which means a client transition **removes the browser's own tab spinner and puts nothing in its place**: shipping `<Link>` alone would have traded a crude signal for none, which is the "impression that the app is not responding" Next's own docs name. The order became layout, then loading states, then `<Link>`, so every step stood on its own.
- **A BASELINE FROM ANOTHER SESSION IS NOT A BASELINE.** Slice 4 appeared to regress TTFB by 90 ms against slice 1's numbers, and two hypotheses were built and killed before an A/B in one sitting showed the branch was 4% FASTER than main. The machine had run a dozen builds in between. Measure both sides in one sitting or do not compare.
- **AN INSTRUMENT IN A BACKGROUNDED TAB LIES CONFIDENTLY.** The first timings came back as 1000 ms and 2000 ms on four consecutive hops; that uniformity was the tell. Chrome throttles `requestAnimationFrame` and timers to about 1 Hz in a background tab, so every rAF-based reading was measuring the throttle. `MutationObserver` is not throttled that way and is the instrument to use. A related trap: clicking before hydration finishes makes a `<Link>` look like a hard navigation.
- **A GREP IS BLIND IN ONE DIRECTION.** The contrast sweep filtered lines containing both `border-cardline` and an element keyword, found twelve boundaries and looked complete. Measuring a real input in the running page returned 1.25:1: five controls on one screen were untouched because their classes live in a shared `const` nowhere near the `<input>`. The signal that works is a `border-danger` ternary, since nothing but a control has an invalid state. W4.7's own lesson, in a new costume.
- **W4.7 SWEPT THE MOBILE APP ONLY, and the dashboard inherited none of it.** Its `controlline` token was created precisely for "an outline button" and "an empty text field", and every one of the sixteen control boundaries in the dashboard sat on the decorative hairline at **1.25:1** until this item moved them to **3.00:1**. When a hardening item names "the app", say which one.
- **A LOADING STATE IS NOT A SCREEN, IT IS A SEGMENT AND EVERYTHING UNDER IT.** `events/loading.tsx` draws its list skeleton over `/events/new`, which is a form, so eighteen nested routes need an override. The frames' own rule ("skeletons at real dimensions, so nothing jumps") is what makes that a defect rather than a detail.
- **Four loading frames were drawn when those screens were designed and never built.** They sat in `dashboard.html` unused while `25` §4 item 4 went unmet for the whole of the dashboard's life, because `17` said nothing about loading and the acceptance criteria could all pass without it. A rule stated in the build process but in none of the specs the builder reads is a rule that does not exist.
- **The spec said 1280 covers everything wider, and it does not.** `dashboard.html` claimed "if it works here it works on anything bigger". An unbounded flex column does not break above 1280, it degrades steadily: on Ayo's 2560px monitor the content column was **2265px** and a 520px form left 1745px of empty space. A layout has a widest sensible width as well as a narrowest.
- **Suite flakes worth knowing before chasing one.** Running both vitest projects concurrently against the one local Supabase produces intermittent failures in `server` tests (six different ones across runs, none reproducible in CI or when the project runs alone), and they get worse under memory pressure. `pnpm vitest run` and `--project server` both passed while `pnpm --filter dashboard test` failed twice.

**W4.13 · The dashboard stops waiting on the server · COMPLETE (2026-09-11)**, one PR. W4.12 fixed how the dashboard NAVIGATES; this is the same complaint on the other two axes, taken off that item's deferred backlog at Ayo's ask: what happens when you press a button, and how many times the server talks to the database before it can answer. No new surface, no migration.
- **The region was the biggest single number in the item and had nothing to do with the code.** `agbc-dashboard`'s functions ran in **`iad1`, Washington**, against a database in **Frankfurt**, and nobody had chosen it: `iad1` is Vercel's documented default, and with no `vercel.json` in the repo the setting existed only in a console nobody had read. Every page was paying two serial auth round trips plus one to four data waves, each crossing the Atlantic twice. Pinned to **`fra1`**, which IS `eu-central-1`, the same AWS region as `agbc-production` rather than merely the same continent. **It is the one change here a local measurement cannot see**, because a local stack has no ocean to cross. **Measured against production the same day, and the instrument was a deployment Vercel had not yet thrown away**: the pre-merge build stays live at its own URL, so both regions could be driven alternately from one browser in one sitting, which is the only comparison W4.12 left us willing to trust. `/sign-in`, which touches no database, went from a **167 ms** median on `iad1` to **68 ms** on `fra1`, and the region was read off `x-vercel-id` rather than assumed from the config. That 95 ms is the SMALLER half: it is the browser-to-function leg only, and says nothing about the function-to-database leg that `authorize()` pays twice on every authenticated page. **There is no before for an authenticated page and there cannot be one now**, because the region moved when the merge deployed and Hobby padlocks p75 duration behind Observability Plus, so 157 to 242 ms median on production is recorded as the baseline for the NEXT change rather than as this one's result. Numbers in ADR 0025.
- **A DECIDED ITEM NOW LEAVES THE MODERATION QUEUE BEFORE THE SERVER ANSWERS** (`useOptimistic`), and comes back if the server disagrees. Measured in a production build: **9 to 21 ms** from click to the row leaving, and **254 ms** for a refused one to return carrying the author's new words. The refusal is real rather than theoretical: it is the compare-and-set `17` §1 has always promised, and it was driven by editing the row behind the reviewer's back.
- **This is the first dashboard screen whose outcome does not travel in the URL.** `?outcome=approved` plus a redirect is how all six surfaces reported themselves, and it cost a document round trip per decision. Spending it here is deliberate and NARROW: the other five move when there is reason to touch them, not in a sweep for tidiness, and the inconsistency is recorded in `17` §1 rather than left to be discovered.
- **The waterfalls were all the same shape: a read that depended on nothing, waiting behind one that did.** The moderation queue signed its photos after two name lookups it needs nothing from (three serial waves → two). The reports inbox awaited its month counter inside the returned object literal, at the bottom of a six-wave chain. Three branch screens found the headquarters by loading EVERY branch, which through `loadMemberCounts` means a row per live profile in the church, to read one name and one id.
...
- **The two halves are independent, which is why the item does both.** All 31 routes are `force-dynamic`, and per Next 16's own shipped docs a dynamic route is **not prefetched at all** without a `loading.js`. Three filter-chip sets are already `<Link>` (`academy`, `verses`, `sermon-audio`) and still feel like full loads, because React blocks on the RSC fetch showing nothing. `<Link>` alone would not have fixed this.
- **The spec forbids the auth "fix", and the next session should not re-propose it.** `17` line 13 requires `authorize()` to keep `getUser()` rather than a locally-decoded `getClaims()`, "so a signed-out or revoked session dies immediately rather than at token expiry" (`02` line 41 generalises it). Its 2 serial round trips stay. `authorize.ts` is not touched by this item.
- Build: 0 a separate `fix/branchform-row-keys` (not part of the item; see below) · 1 baseline timings on `next start` · 2 pending states · 3 `<Link>` · 4 the shared `(dashboard)` layout · 5 loading states · 6 close out (region, ADR 0025, `17`, `credentials.md`, re-measure).
- **A data-loss bug found while mapping, fixed separately and first.** `BranchForm.tsx:217,:358` key repeatable service and leader rows by array index over uncontrolled `defaultValue` inputs and remove by index, so removing a middle row re-keys everything below it, React reuses the DOM nodes, `defaultValue` never re-applies, and typed values do not follow their row. Live for any admin editing a branch.
- Done: timings recorded before and after (not "feels faster"); screenshots of every visible change in both themes against its frame; a narrow window; `aria-current` surviving the `<Link>` conversion; every route path in the build output unchanged after slice 4.

**W4.15 · Share as a picture · COMPLETE (2026-09-14)**, five PRs in this repo (#298 slice 1, #300 slice 2, #301 slice 2b, #302 slice 3, then slice 4) and two in the website (`agbc-website` #49, #50). Ayo, 2026-09-12: "the share button currently is text only, but we need it to be in picture form". All nine share points (the verse, three testimony surfaces, the prayer request, the event, the message, the branch, the milestone) now open `SHARE-PREVIEW`, a sheet with a 1080 × 1080 card that leaves the app as a PNG, with "Send as text instead" keeping the words every surface sent before. Plan at `docs/spec/plans/W4.15-share-as-a-picture.md`, which carries what it decided; the eleven cards and the sheet are frames in `entry-flow.html` under `W4.15`.
- **The card is a PNG, so nothing on it is clickable, and `expo-sharing` sends a file and nothing else**: no caption, no link, no app name travels beside the picture on Android (`react-native-share` can carry both, but WhatsApp drops one). So the QR is the only element a recipient can act on, and the item's second half (slice 2b) made it open the THING that was scanned: `https://www.agbcglobal.com/app/t|p|e|m|b/<id>` on `www` (the apex 308s and is not claimed), routed by `app/+native-intent.ts` through the same pending-link store a notification tap uses, opened for guests too. The website serves `/app/*` for phones without the app. **Short segments by measurement**: `/app/t/<uuid>` is a version-4 code that decodes as far down as the 22-byte front-door code did; the long form tipped into version 5 and failed at 600px.
- **The capture is at natural pixel size**: `react-native-view-shot`'s `width`/`height` RESIZE the result, so the card renders at `1080 / PixelRatio` dp and captures at exactly 1080 with no resample. The card is not theme-aware (it lands in somebody else's theme), every colour a literal in the `shareCard` token group, asserted in the contrast contract.
...
**W4.16 · The rhythm keeps a real calendar · BUILT, MERGED BUT NOT SHIPPED (2026-09-14)**, five PRs: #305 the plan, #306 this entry, #307 the database (slice 1), #308 the frames (slice 2), #309 the app (slice 3), then the close-out. It changes nothing for a member until two things happen together: the store build carrying #309 is submitted, AND `supabase-deploy.yml` is dispatched so production gets `20260914120000` and `20260914140000`. **Do not dispatch `supabase-deploy.yml` for any reason before that build is submitted** without deciding it first: it would carry both migrations ahead of the app that reads them, and installed builds 22 and 23 would draw "Next milestone: 4 weeks" and a full ring at four weeks while the server no longer awards anything there (plan §5, which also asks for a read-only count on production of who holds the four week-rhythm kinds before the dispatch, with Ayo's OK). Plan at `docs/spec/plans/W4.16-rhythm-real-calendar.md` (it stays until the item ships; read it before touching milestones, Home's rhythm ring or RHYTHM's Next card), frames in the `W4.16` section of `entry-flow.html`, rules in `10` §Milestones. The same store train also carries #264, the Expo SDK 57 alignment and React Native 0.86.3 (the Hermes memory-regression fix), which changed native code: the dev clients need a new EAS development build before the next device session, deliberately not yet spent (Ayo, 2026-09-14: the monthly quota is kept for the Android and iOS release builds).
...
- Done: the plan's pgTAP cases green and each mutation-checked (**met**: `056` with 69 assertions, `030` and `031` rewritten onto fixed dates, 24 database mutants and 8 app mutants each killed); every progress shape on the device against its approved frame across the full size, scale and theme matrix (**met**: the S22 at 411dp and 360dp, text 2.0 browsing AGBC Lighthouse Berlin, English and German, both themes; the Tab S10+ in both themes; iOS owed, as for every item without an iPhone); the migration dispatched to production together with the app build that carries slice 3, never before it (**owed**, and the one thing between this entry and COMPLETE).
- **Six lessons, the first two the ones worth carrying furthest.**
  - **A name that states a length of time must be measured in that time.** Every label on the ladder was calendar language and every threshold a count of attended weeks, and the two only disagreed on some months and some patterns of attendance, which is how it survived W2.8's review. The fix was not new thresholds but a different unit.
  - **A rule that differs by date needs tests and seeds that are date-proof, not now()-anchored.** The old `030`/`031` built history backwards from today, so under the calendar rule they would have passed on some days and failed on others. `056` asserts on fixed dates, the progress helper takes `today` as an argument, and the dev seeds were checked through the real functions against every reset day of 2026 and 2027 before they were written (Grace held the month on all 730; Tobi always counted this month).
  - **Longer copy is a layout change.** "2 of 4 in September" replaced "6 to go", and at the S22's largest text on 360dp the Next card broke its title mid-word, because React Native does not shrink a flex child unless told. Frames at 1.0 and green tests could not see it; the device matrix did, and the row now wraps.
