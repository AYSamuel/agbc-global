...
**W4.0 · Link a website registration by hand (#164)** (backend landed 2026-08-31; the screen is owed)
...
- **Owed: the screen.** There is no mockup frame for any of it. Per the standing rule the frames come FIRST and need Ayo's approval before a line of screen code: the queue populated, the queue EMPTY (the state an admin will see most weeks), the link flow with its suggestions and their stated reasons, the confirm, and the set-aside state.
...
- Done: an admin links a payment record to a member from the dashboard, the member is told, and a wrong link can be undone.
