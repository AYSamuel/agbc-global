...
- Not done, honestly: a physical phone for slices 3 and 4 (the phone layouts were the tablet's display override), TalkBack itself on the bar and the segment, and the metadata refresh on the device (unit-proven only). The two hand-inserted audio-only rows on the local stack go with the next `pnpm db:reset`.
