...
- **Six lessons, and the first two generalise furthest.**
  ...
  6. **`02` described a bucket that has never existed.** `avatars` is in the storage table and in no migration, harmless only because nothing writes `profiles.avatar_url` yet. Recorded there with what it costs the day an uploader is built.
