...
keeps a real calendar, planned 2026-09-14), whose slices 1 to 3 (the database, the approved frames and
the app) are done, so its slice 4 (the close-out docs) is the next thing to pick up. Past
...
**W4.16 · The rhythm keeps a real calendar** (**IN PROGRESS**: slice 1, the database, landed 2026-09-14 in #307; slice 2's frames were approved as drawn the same day (the `W4.16` section of `entry-flow.html`); slice 3, the app, was built and verified on both devices the same day, with the gathering ladder's grant folded in; the close-out docs are next. Plan at `docs/spec/plans/W4.16-rhythm-real-calendar.md`, merged in #305: read it before touching milestones, Home's rhythm ring or RHYTHM's Next card, because it carries every decision and the reason for each, and its §4 "As built" carries what slice 1 settled. **Do not dispatch `supabase-deploy.yml` for any reason until slice 3's build is submitted** without deciding it first: the dispatch would carry this migration to production ahead of the app that reads it, plan §5.)
