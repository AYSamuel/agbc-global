...
- **"I'm here" while browsing another branch** records attendance AT the browsed branch (real visits happen: diaspora members travel); streaks count attendance at any branch (week = the ISO week of `service_date`, fixed at write time, `10`). **The tap ASKS FIRST whenever the branch under it is not the member's home branch** (added 2026-09-07, after a member checked in at a branch he was only browsing). The visit note has said where the tap lands since W2.8, but a note DISCLOSES and never asks, and the tap writes something no member can undo: `attendance` is unique on `(profile_id, service_date)` and members hold INSERT only, so a tap made while browsing credits the wrong branch AND spends that member's one check-in for the day, leaving their own branch without the row they earned. The sheet names the branch, says there is one a day, and its refusal puts the chip back on their own branch rather than merely closing, because "no" would otherwise leave them on somebody else's front page with their own service behind the chip (`04` forbids dead ends). Nothing is written until it is answered, and a member standing in their own branch is never asked. **The question belongs to the app rather than to a screen**: `BRANCH-INFO`'s check-in raises the same one (a member can reach ANY branch's from the map or the list, with the chip pointing elsewhere), and so does the gate-return replay, which fires after AUTH-4 has already moved the member and given them the home branch they chose seconds earlier. So the sheet is mounted at the root beside the milestone celebration and the notification ask, and keyed to the tap rather than to the chip. It is NOT a location check: `10` turned geofencing and QR codes down for warmth, and attendance stays a member's word, now asked for out loud on the one occasion where the answer is not obvious.
...
- **Revisited and upheld 2026-09-15 (Ayo).** For one morning the daily verse ran NKJV, Schlachter 2000, the Herziene Statenvertaling and Segond 21, and then each publisher's own terms were read. **NKJV** (HarperCollins Christian Publishing) caps gratis use at 500 verses and at Scripture being 25% of the work, excludes a verse that "stands alone" such as artwork (which is what a share card is), and sends smartphone apps to its licensing department: an app needs written permission. **Segond 21** allows 500 verses but not more than 50% of the work, with the notice on a title or copyright page. **The Herziene Statenvertaling** is free for non-commercial use short of the whole translation, always with "Copyright ©2010/2016 Stichting HSV" shown. **Schlachter 2000**'s publisher states no allowance at all. A verse-of-the-day surface is almost entirely Scripture and grows by a verse a day, so every one of these limits is a running total the app would cross. All 64 rows went back to this table the same day. Changing it means written permission first and an in-app credits surface second (a `LEGAL` screen under Settings, holding each publisher's notice in its own words, was scoped for it and not built), in that order.
