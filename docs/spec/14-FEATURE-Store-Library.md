...
## Data
- `books`, `entitlements`, `reading_state`, `payhip_events`, `unmatched_purchases` (see `02`), all built in W4.1 slice 2 (`20260901120000`). Book files in a PRIVATE bucket, covers in a public-read one (`book-covers`, added in the same slice: the cover is the advertisement, not the asset). Accept that a downloaded offline copy leaves our control (unavoidable); never issue a shareable long-lived URL.
- **The entitlement check is an RLS SELECT policy on the bucket, not an edge function** (decided W4.1 slice 2, correcting this line). `createSignedUrl()` only works for a caller the policy admits, so ownership is enforced once, where the bytes leave from. This is the same correction W2.3 made to `testimony-photos`, which this doc's sentence predates: the policy gives the identical guarantee with no extra service on the read path and one less place to re-implement the rule wrongly. The policy also requires `revoked_at is null`, which is how a refund closes READER and not only LIBRARY.
