...
- **Artwork (W3.1 slice 5, 2026-08-15):** a message's own picture lives in `artwork_path`, an object path in the **public-read** `sermon-artwork` bucket, and is preferred over `thumbnail_url` on every surface that draws a message's picture: Watch's hero, every rail/search/MY-LIST row, the player's artwork slot, and the lock-screen metadata the OS renders. A message with neither falls back to the branded gradient the player has drawn since W1.3, which is a designed state, not a failure. **Never `thumbnail_url` itself:** the nightly sync owns that column and overwrites it, so anything we put there dies on the next run. The posture is deliberately NOT the audio's, and `02` §Storage carries the full argument; the short version is that the audio is the asset and the artwork is the advertisement for it, and a rotating signed URL would defeat both the device image cache and the OS's own out-of-process fetch of the lock-screen art. **Which picture wins is decided in one place** (`features/watch/artwork.ts`) because four surfaces ask.
