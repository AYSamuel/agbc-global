...
- Actions: play/pause/seek, ±15s, speed (1x/1.25x/1.5x), **Add note** → `SERMON-NOTES` (gate), **Save** (gate), **Share** (OS/WhatsApp), **Open on YouTube** (fallback). The transport (scrub, ±15s) belongs to AUDIO mode alone: while the YouTube embed is up it draws its own controls (W1.3), and speed is dimmed with it. The scrub SHOWS progress and takes no drag; ±15s is the seek, which is also the tap alternative a drag would owe. **Speed took the mockup Download tile's slot** (W3.1 slice 3): nothing in this document ever specified a download, that tile had sat dimmed since W1.3, and speed had no home in any frame. A dimmed tile still answers a tap with the reason it is dim, rather than announcing itself disabled and saying nothing.
