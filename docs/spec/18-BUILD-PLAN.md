...
## Launch checklist
- [x] ~~Apple: **reuse the existing app record** (bundle id `com.olayinkaademiluka.grace-portal`) under the existing (non-profit) team; listing updated to AGBC Global branding; privacy labels; account-deletion present; screenshots. See `19`.~~ **DONE: AGBC Global 1.0.2 (24) is LIVE ON THE APP STORE since 2026-09-16** (W4.17; submitted 2026-09-15, approved the same day, released by hand into 174 countries). Name AGBC Global, **English listing only** (Apple refused the name in German, `store-submission.md` §3b), 16+, privacy labels published, 5 iPhone and 4 iPad screenshots, China mainland excluded. **What this line does NOT cover and stays open below: the iOS E2E**, because the release shipped without an Apple device in the project. The record exists (Apple ID `6760579106`, live as Grace Portal 1.0.0 (18), iPad included). **Decided by Ayo the same day: iOS ships without verification on an Apple device**, because there is none in the project. What that accepts is written in `docs/spec/plans/W4.17-ios-release.md` §1; the iOS E2E line below stays open because of it, not in spite of it.
...
- [ ] **Push: FCM done, APNs owed.** Split 2026-08-31 because half of it has been proven and the other half cannot start. **FCM V1 is configured and PROVEN end to end**: real pushes reached the physical Android device through Expo -> FCM across all ten notification tiers on 2026-08-30 (W3.6 slice 3), with receipts `ok`. **APNs is owed and blocked**: there is no iPhone in the project, so no iOS build has ever existed to carry a key. Same root cause as the iOS E2E below. **Update 2026-09-14 (W4.17): the key is no longer blocked, only unproven.** The Apple team holds no keys at all today; W4.17 slice 2 creates the APNs key into EAS so the first iOS build carries push, but with no iPhone nobody can watch a push arrive, so this line stays open until one does.
- [x] ~~WhatsApp broadcast sender approved~~ (not applicable: ADR 0014 removed the Cloud API and its sender registration).
- [x] ~~Forced-update gate wired: remote minimum-version check (Android in-app updates + config gate on iOS).~~ **Built in W1.2 and verified then by faking the minimum**, `apps/mobile/src/features/update-gate/`. Ticked 2026-08-31 after the audit found it done but unticked. **The production floor was set on 2026-09-08** to `{"ios": "1.0.0", "android": "1.0.0"}`, replacing the `0.0.0` placeholder the 2026-08-18 seed left. It still lets the published build through, because 1.0.0 IS the published version, and that is the only safe value while it is the only version in the wild: anything higher blocks build 22 itself with nothing to update to. It becomes a lever the day a newer version is live, which **1.0.1 makes true for the first time**: raise Android's floor to 1.0.1 only once the Play console shows that build live, and leave iOS where it is, because iOS has never had a build at all. **A build-level floor is not expressible today**, and asking for one is the natural mistake: `parseVersion` accepts `x.y.z` only, the gate compares against `expoConfig.version` (`1.0.0`) and never reads `versionCode`, and an unparseable value FAILS OPEN by design, so `1.0.0+22` would sit in the table gating nothing. Teaching the gate `x.y.z+n` remains unbuilt and is written up in W4.10's plan as the one thing still open there.
- [x] **A member is told about an ordinary new version, and can install it without leaving the app.** Nothing on this checklist had ever asked for it, and the app had only the wall above: the ONLY update a member ever heard about was one that had already locked them out. **Built in W4.10 (2026-09-08):** a once-a-day dismissible sheet, Play's flexible flow behind its button (download in the background, relaunch into the new version), Play's immediate flow at two releases behind, and the wall's own button now installing instead of linking out. **iOS is inert until the App Store record's numeric id exists** (`ios.infoPlist.AppStoreID`), which is the same blocker as the store link in `lib/links.ts`; the check answers "nothing new" safely until then. **The id was read on 2026-09-14 (`6760579106`) and W4.17 puts it in 1.0.2**, so the first iOS build carries the check switched on; it cannot fire until a version newer than 1.0.2 is on the App Store, and the store link in `lib/links.ts` still waits for AGBC Global to be live there. **And none of it can be verified before the release AFTER the one that ships it**, because Play's in-app updates only work on a Play-installed build.
- [ ] **Rotate the Supabase CI access token before 2027-09-16.** It is the credential the production deploy runs on (`SUPABASE_ACCESS_TOKEN`, the `production` environment), it is account-wide, and **one year is the longest Supabase will issue**. It lives here rather than only in `credentials.md` because the last one expired on 2026-09-17 against a calendar reminder that nobody saw, and was discovered by a deploy that would not run (W4.21, 2026-09-18). The failure is silent, late, and lands on whoever next needs to ship something urgent
...
- [ ] **Audio owner AND audio SOURCE assigned**: weekly sermon MP3 upload is someone's named job (`08`), **and the file is MP3 under 50 MB** (W4.9 slice 1: the shelf refuses anything else, in the browser, with the reason; 64 to 96 kbps mono keeps an hour under the cap), and the job needs a source `08` never names. **Not YouTube.** `08` already forbids the app extracting or proxying a video's audio track, and copyright ownership does not grant a download right, because YouTube's terms govern access to the service separately from who owns the content. The two legitimate sources are the media team's ORIGINAL recording (better quality, no terms question, and the only one that scales to a weekly task) or YouTube Studio's own download of your own upload. Raised 2026-08-30 when W3.6 slice 4 needed a file and there was none.
- [x] **YouTube brand asset swap** (`08`, added W3.1 slice 4, 2026-08-15): **DONE 2026-09-09.** The badge is now YouTube's own Core icon from brand.youtube (`youtube-icon{,@2x,@3x}.png`, colours untouched), replacing the one we drew, whose red was the old #FF0000. Placement and link unchanged; the gap to the label widened to their clear-space rule. It is a JS-side change and ships with the next build like everything else.
- [x] **Play Console `mediaPlayback` foreground-service declaration** (`08`, added W3.1 slice 3): the app now holds `FOREGROUND_SERVICE_MEDIA_PLAYBACK` (expo-audio's config plugin) and uses it for background sermon audio, which the app-content form must declare or the release is rejected. Nothing to build; someone has to fill the form. **Missed at the 2026-09-05 submission, flagged overdue by Play the same day, and FILED that evening**, which restarted the review. The form is one checkbox plus a REQUIRED video link showing the feature in use, so it could not be filed until production held a sermon with audio; one audio-only message was uploaded for it. The record, the video and what the next release repeats: `docs/runbooks/store-submission.md` §SUBMISSION RECORD, follow-up 1.
...
- [x] Restore drill executed (prod dump restored into a scratch project, `21` §7). **DONE 2026-09-09: last night's nightly restored from B2 with the vault's key, 73/73 tables and both website checksums identical to production, dashboard booted and signed into with the restored MFA factor; `docs/runbooks/restore-from-backup.md` §Drill record carries the timings and four findings.** The target was the LOCAL stack rather than a scratch project, because Free allows none (the runbook's "Path A on the local stack"); next drill due by 2026-12-09. It had been **never run** (audited 2026-08-31). Note the dependency the audit surfaced: `21` §7 asks the drill to "boot the dashboard against it", so this cannot be completed until the dashboard is deployable. A restore needs a free ACTIVE project slot on the Free plan; **the old paused project held the second one until it was deleted on 2026-09-09 (Track P Phase 5), so the slot is free**, and the dashboard has been deployed since 2026-08-31. Nothing blocks this drill any more.
...
- [ ] **iOS E2E owed:** the Maestro journeys and the Family loop have run on Android only (no iPhone in the project at W2.10). Run the suite on an iOS build once an Apple device exists; `21` §4 asks for both platforms pre-release. **The first iOS release (W4.17, 1.0.2) goes out WITHOUT this, by Ayo's decision on 2026-09-14**, so the suite's first iOS run will be against an app already on the store.
...
- [ ] **Push tray icon (Android): BUILT 2026-09-09, verification owed on the next Play-installed build.** `notification-icon.png` (96x96, white on transparency, the icon set's own letterform at ~80% of the canvas, rendered by `scripts/render-app-icon.sh` and measured) is wired as `expo-notifications`' `icon` beside the gold `color`. It is a native asset, so no dev client or build 23 shows it; the acceptance test is a real push on the next store train's build, looked at in the shade and the status bar, exactly like W4.11's R8 walk. Until then Android keeps falling back to the app icon, which rendered correctly on device (2026-08-16). The reasoning sits in `app.config.js`'s plugin comment.
