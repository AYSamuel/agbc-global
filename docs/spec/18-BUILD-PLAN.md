...
## Launch checklist
- [ ] Apple: **reuse the existing app record** (bundle id `com.olayinkaademiluka.grace-portal`) under the existing (non-profit) team; listing updated to AGBC Global branding; privacy labels; account-deletion present; screenshots. See `19`. **SUBMITTED 2026-09-15 as 1.0.2 (24), waiting for Apple's review, manual release** (W4.17, which shipped the same commit to Play on 2026-09-14): name AGBC Global, English listing only, 16+, privacy labels published, screenshots in. This line ticks at release, step 3 of `docs/spec/plans/W4.17-ios-release.md` §6. The record exists (Apple ID `6760579106`, live as Grace Portal 1.0.0 (18), iPad included). **Decided by Ayo the same day: iOS ships without verification on an Apple device**, because there is none in the project. What that accepts is written in `docs/spec/plans/W4.17-ios-release.md` §1; the iOS E2E line below stays open because of it, not in spite of it.
