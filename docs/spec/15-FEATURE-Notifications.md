...
| **Personal: activity** | The individual | Someone prayed for you, Glory reactions on your testimony | `prayer_activity` / `testimony_activity`. **Produced by `activity-notices` since W3.6 slice 2 (2026-08-29), and by nothing at all before it**: every other part of this tier had shipped with W3.3 (the type values, the channels, the templates in four languages, the app's renderer, both switches on NOTIF-PREFS) and no code anywhere wrote the rows, so these two switches gated nothing for six weeks. Two decisions were taken with it. **"Someone prayed for you" fires on "I prayed", not on "I will pray"**: the template is past tense and should tell the truth, and the cost (a commitment never fulfilled notifies nobody) is what the three nudges below already exist for. **Glory collapses per testimony per HOUR, not per day**, with a settle window so a burst is a single notification carrying the true size of the burst. Per-day was the original wording here and it both undercounts and silently drops: a testimony that keeps drawing reactions would tell its author "3" while everyone after the first hour went unmentioned, which is the opposite of what this tier is for. The hour is an argument rather than a constant, so widening it costs a call site and not a migration |
...
- List of `notifications` (rendered per the user's language, time, read state), newest first. **Cursor pagination** (~30 per page); unread badge caps at "99+"; activity batching: "N people said Glory" collapses per post per **hour** (amended 2026-08-29, W3.6 slice 2, from "per day"; see the tier table's activity row for why); a footer row at the retention boundary: "Older notifications are removed after 12 months."
...
- **Share links (W4.15 slice 2b):** a share card's QR encodes `https://www.agbcglobal.com/app/t/<id>` (testimony), `/app/p/<id>` (prayer request) or `/app` (the verse). With the app installed, Android opens the URL in the app through the App Links claim above, and `app/+native-intent.ts` turns it into `/testimony/<id>` or `/prayer/<id>` (`features/share/links.ts`, the same conservative segment check as the notification allowlist; anything else under `/app` lands on Home). Without the app, the website's `/app/*` page says what the app is and sends the reader to the store. A cold-start link rides the same pending-link store as a notification tap, with one difference: it is opened for a **guest** too, because a testimony and a prayer request are guest-browsable and the scanner is more often a stranger than a member.
