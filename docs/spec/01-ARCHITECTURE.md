...
| Server logic (moderation actions, push fan-out, receipts) | **Edge Functions** (Deno) |
...
- **Background jobs**: push fan-out, scheduled verse-of-day, receipts (a queue: BullMQ/Redis, or serverless cron).
...
- Broadcasts cost nothing to send: push + in-app only, no per-message billing anywhere in the product (ADR [0014](../decisions/0014-push-only-broadcasts.md); the WhatsApp Cloud API and its ~$200-per-blast pricing were dropped 2026-07-29).
...
| ~~WhatsApp Cloud API~~ | ~~Broadcast~~ | **Not used** (ADR 0014). Sharing individual content out to WhatsApp is the OS share sheet, which needs no vendor integration |
...
- **Queued (idempotent one-tap actions):** attendance "I'm here" (carrying `client_taken_at`, see `02`), plan-day complete, RSVP status, Glory toggle, prayer commit/fulfil ("I will pray" / "I prayed"), My List save/unsave (W3.1). Offline: enqueue locally (persisted), show optimistic UI, replay on reconnect. **Playback position is deliberately NOT queued** (amended 2026-08-15; this list said it was until W3.1 built it the other way): it is one value superseding itself every ~10 seconds, where a failed write costs nothing because a newer sample follows and the device-local layer never missed it. It rides its own throttled direct write (see `08`); queuing it would flood the queue with wishes whose only correct resolution is to be thrown away. **The queue stores desired END-STATE per (action, entity), not an op log:** the last tap wins locally and exactly one idempotent write replays per entity (a tap-untap-tap sequence replays as one "reacted" write; ordering races cannot occur). Safety: each write is idempotent server-side via the unique constraints in `02`. Conflict policy: server state wins; a rejected replay reconciles the UI quietly. The queue is small and capped: eviction removes whole entities, oldest first, and REVERTS that entity's optimistic UI on eviction; cleared on sign-out.
