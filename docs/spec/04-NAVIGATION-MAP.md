...
- **`CONTACT`**: contact form (name, email, message → edge function → church inbox, reply-to = the sender's email) + direct email links per branch. Submit → confirmation; failure preserves the draft. **A message sent once arrives once (W4.18 slice 3, 2026-09-16):** every send carries an `Idempotency-Key` minted on the phone and kept by content (`lib/contactForm.ts`), which the function forwards to Resend, which refuses a second email under it for 24 hours; the same words retry under the same key, changed words mint a new one. Both callers, this screen and the Academy's REGISTRATION-CONTACT sheet, go through that one sender. An answer that never arrives is reported as unconfirmed, never as "offline": the email may have gone, and the retry is safe. Plan: `docs/spec/plans/W4.18-a-message-sent-once.md`.
