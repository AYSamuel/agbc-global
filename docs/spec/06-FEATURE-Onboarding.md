...
**All three triggers exist as of 2026-09-07; until then only the first did, and the other two were assumed rather than built.** The cost was total for anyone they were meant to catch: a member who never checked in and never RSVPd was never asked, so the OS was never asked either, and no push could arrive while the notification centre filled up. `15`'s "push is off" banner could not help, because it waits for this app to have had its one ask, so the app was silent from both ends and the member had to find the OS setting alone (Ayo, over several days). Three notes on the shapes now built. The sign-in ask is raised **when Home is FOCUSED**, not when a session appears: tab screens stay mounted, so a member who signs in from MORE would otherwise meet it over the More list. The Settings-toggle trigger goes **straight to the OS dialog with no pre-permission sheet**, because flipping a switch that says "service reminders" is itself the explanation and a sheet would ask the same question twice; it marks the ask flag all the same, since that flag is what `15`'s banner keys on. And **a grant now registers the push token immediately** (`features/notifications/permission.ts`), because the runtime otherwise fetches one only when a member APPEARS, which for every ask after sign-in has already happened: permission granted, nothing registered, and no push until the next cold start.
