...
**Grants are the OTHER half of the boundary, and they are not automatic** (corrected 2026-08-20, `20260820200000`). Supabase's project bootstrap carries `alter default privileges in schema public grant all on tables to anon, authenticated, service_role`, so a table created without explicit grants starts life handing BOTH API roles INSERT, SELECT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER and MAINTAIN, and **an explicit `grant select` on top does not displace it**. Ten tables and three views from W0.10/W1.x were in that state until W3.5 slice 5a tripped over it while writing column grants that changed nothing. Nothing was ever open (FORCE RLS plus correct policies refused every write, and TRUNCATE, the one privilege RLS cannot filter, is not reachable through PostgREST), but RLS was the sole boundary where this doc asks for two. The default privilege is now revoked for the roles we own, every one of those objects carries grants derived from its own policy set, and `supabase/tests/048` asserts the invariant SCHEMA-WIDE rather than per table, so the next table is covered without anybody remembering. **When adding a table: write its grants, and expect `048` to fail if you do not.**
...
| books, refined W4.1 slice 2 | SELECT **where `published_at is not null`** | same, **plus any book they hold an entitlement to** (so unpublishing never empties a Library) | no leader surface: books are global content (`17` §Roles) | SELECT all + INSERT + UPDATE through RLS; no DELETE for anyone |
| branches / branch_services | SELECT | SELECT | SELECT only: branch management is ADMIN-only (`17` §5), so a leader gets no write version of it | admins INSERT/UPDATE at `aal2` (branch_services also DELETE), through a COLUMN grant that excludes `status`, `is_hq`, `archived_at` and `archived_by`, so neither archiving nor moving the headquarters can be reached by writing a column: each has to go through its function (`archive_branch()`, `set_headquarters()`), which is where the preconditions live. `slug` is in the INSERT grant and not the UPDATE one, so a branch is named once. No DELETE on `branches` for anyone: they are archived, and the absent grant is the enforcement. Service role: all |
...
| `book-files` | private | signed URL per request after an `entitlements` check, short TTL (minutes); see `14`. **Built W4.1 slice 2, and the check is an RLS SELECT policy rather than an edge function**: `14` described a per-request function, and W2.3 overturned that same wording for `testimony-photos` with a reason that transfers wholesale (the policy gives the identical guarantee with no extra service on the read path, and one less place to re-implement the rule wrongly). The SELECT policy IS the mint permission, and it joins `books` to `entitlements` with `revoked_at is null`, so a refund closes the file as well as the shelf. 100 MiB, `application/pdf` + `application/epub+zip`. `anon` is deliberately absent, unlike `sermon-audio`: a book is not guest content. Writers are live-table admins at `aal2`, machine-minted `<uuid>.<ext>` names, no UPDATE policy |
| `book-covers` | public-read | **W4.1 slice 2, and not in this table before it.** `sermon-artwork`'s posture and limits exactly (5 MiB, jpeg/png/webp, machine-minted names, write-once, referenced objects undeletable, admin writers at `aal2`), for the identical argument: the cover is the ADVERTISEMENT for the asset, on every card in a guest-first grid, so there is nothing in it to fence and all three costs of a signed URL apply (image caching by URL, URLs that must not be persisted into an offline query, and a CDN that cannot hold a rotating signature). The alternative was hotlinking Payhip's own CDN, which would put a third party's URL on every card in the store |
| `avatars` | public-read | low sensitivity; still re-encoded on upload. **CREATED W4.5 slice 5 (2026-09-02), having been described here and in no migration since W1.2.** Found while walking the deletion reach: `profiles.avatar_url` existed, `authenticated` held a column UPDATE grant on it, and the erasure recorded its value for `erasure-sweep`, which would have called `remove()` on a bucket that was not there, thrown, retried five times and raised a `job_alerts` row about a GDPR obligation nobody was working on. 5 MiB, jpeg/png/webp. The writer is the MEMBER, not staff, which is what makes this the only public bucket here without an `aal2` clause (there is no step-up in the app to demand) and the only one folder-scoped: objects live under `<profile_id>/`, `testimony-photos`'s rule with the moderation gate taken off. Write-once as everywhere else, and it matters most here, because a public URL is cached by the CDN and by every member's device. Creating the bucket is also what made `avatar_url` dangerous for the first time, so the same migration adds `assert_avatar_path_owned()` to `profiles_guard`: without it a member could point their profile at a stranger's object and have their own erasure delete it, which was mutation-checked rather than assumed. **The column's contract is ENFORCED rather than described** (`20260902130000`): `assert_avatar_path_owned()` refuses a URL outright. The name `avatar_url` predates the W2.3 rule that a row holds a PATH, and the hazard was never the name but what the column could hold: `erase_profile` records this value for `erasure-sweep`, which skips anything URL-shaped rather than guessing a bucket out of a hostname, so a URL here means an erasure that reports success while the member's face stays in storage for ever. Renaming would document that; the check makes it impossible, with no view to recreate and no generated type to regenerate. Do the rename anyway the day the uploader is built and is already editing all of them. **Still owed by that uploader** (`16`'s profile edit; nothing writes the column yet and the app draws a gradient with initials): the re-encode and EXIF strip this row has always promised, which needs an upload path to exist before it can be applied |
...
| `event-images` | public-read | **W3.5 slice 4b.** `sermon-artwork`'s posture and limits exactly (5 MiB, jpeg/png/webp, machine-minted `<uuid>.<ext>`, no UPDATE policy, referenced objects undeletable), for the same reason: it is the picture an event advertises itself with, on a page a guest can open without an account. ONE difference, and it is who writes: a branch's own **leaders** run its events, so the policies read `caller_is_moderator_live()` rather than `caller_is_admin_live()`. That widens who may put bytes in the bucket and nothing else: WHICH event a picture lands on is still the `events` row policy's decision, per branch, and that refusal is SILENT (RLS filters the update; pgTAP `050` asserts the silence). The bucket is deliberately not folder-scoped by branch: it would only decide which leader may delete an orphan, and would buy a permanent branch id in a public URL for that |
| `sermon-audio` | private, signed URLs (24h TTL) | **Decided 2026-08-14 (W3.1, with Ayo):** the audios are the church's own uploads (some sermons are never on YouTube), so a URL copied out of the app's traffic dies within a day instead of becoming a permanent free door; 24h cannot expire mid-listen and the player re-mints on open. **50 MB and `audio/mpeg` only since W4.9 slice 1 (`20260906120000`, decided 2026-09-06)**: 50 MB is the Supabase Free plan's fixed per-file cap, which the earlier 150 MiB row could not raise (the first real upload found out), and MP3 is the one format `08` ever asked for, now enforced at the picker, the mint, the byte check and this bucket alike; the INSERT policy's name rule admits `<uuid>.mp3` only. Writers: live-table admins at aal2 (the aal2 claim is safe here where it was NOT on content tables: no mobile member writes this bucket). Object names machine-minted `<uuid>.<ext>`. SELECT (= the mint permission) covers `anon`: guest listening is a `08` requirement, so the TTL is the whole fence. Write-once like `testimony-photos`; see `08` |
...
| city | text | |
| country | text | |
| is_hq | bool | Glasgow = true. **At most one branch holds it** (`branches_one_headquarters_idx`, a unique index on a constant partial on `is_hq`; a UNIQUE CONSTRAINT cannot carry a WHERE clause). It decides three things that never raise an error when they are wrong: the timezone a ministry-wide event defaults to, the one branch `archive_branch()` refuses to close, and where a member whose branch closes is offered first. It also draws a gold HQ badge in the app's branch switcher and onboarding list, so moving it is visible to every member. Outside both column grants: `set_headquarters()` is the only writer, and it CLEARS before it SETS, because a partial unique index is checked per row and a single statement gives no ordering promise (W3.5 slice 5b) |
| status | enum | `active` \| `archived` (default active). Branches are ARCHIVED, never hard-deleted (attendance, content, audit rows reference them). Archived: hidden from onboarding/BRANCH-SWITCH/BRANCHES/map; its services stop, which is a READ of this column by everything that would otherwise announce one rather than a flag set on `branch_services` (there is no such flag, and `archive_branch()` leaves the schedule exactly where it is, so re-opening restores the diary): `service_reminder_batch` joins on it, Home GOES QUIET while the browsed branch is closed, down to the closure card and the verse: no service card, no rhythm strip, no family, no latest message (Home is the branch's front page, so with no branch there is no front page, only the ask and a verse that belongs to the whole family; it gates nothing, since every other tab is untouched and one tap on Choose gives the screen back), and `attendance_insert_guard` refuses a member's check-in against it. **Until 5c's follow-up only the FIRST of those asked**, so a member whose branch had closed was still shown "THIS SUNDAY · 11:00 AM" with a check-in under it and a "Plan a visit" that dead-ended on BRANCH-INFO's own refusal, one card below the banner saying it had closed. A guest browsing a branch that closes has no such card, so their BROWSE branch falls back to HQ instead: they have nothing invested in it, it has already left the switcher, and an unexplained gap is worse than a quiet move; content stays readable under Everywhere; members whose home branch is archived are prompted on next launch to pick a new one (HQ preselected) and get no branch-tier notifications meanwhile. **That prompt is the one branch change that needs no approval and ignores the cooldown** (ADR 0015): there is no branch left to stay in and no leader to ask, so it is a server-owned assignment writing its own audit row, not a request; residual pending moderation escalates to admins immediately; the dashboard blocks archiving until its leaders are reassigned (`17` §5). **Built W3.5 slice 5a** (`archive_branch()`, `restore_branch()`, `rehome_from_archived_branch()`), **and its member half in 5c**: the prompt is a dismissible launch surface with HQ preselected, backed by a Home card that is NOT dismissible, because once the prompt has been put off nothing else in the app would ever ask again. It deliberately does not gate the app (browsing has never needed a branch), and re-homing also moves the member's BROWSE branch, which is a persisted local snapshot that would otherwise keep drawing a closed branch's service time on Home. Only `archive_branch()` may set this column, because `status` is outside the admins' column grant. Archiving additionally CANCELS the branch's future scheduled events (so `event-notices` tells everyone holding an RSVP) and stops its broadcasts (a `sending` one is halted, one awaiting approval is rejected with a note, and `broadcast_recipients` gives a branch-scoped one no audience at all while a MINISTRY one still reaches these members: the branch TIER stops, not everything). It refuses HQ, the last open branch, and any branch a leader still points at. "Reminders and live windows" above was written before ADR 0021 removed the live layer |
...
| location | text | |
| image_path | text null | object PATH in the PUBLIC-READ `event-images` bucket (W3.5 slice 4b), never a URL: the URL is derivable and a stored one would pin the project host into the row (`19`). The hero behind EVENT-DETAIL's gradient; null is the branded cover, a designed state (`11`). Renamed from `image_url` before it ever held a value, the same way `testimonies` was in W2.3. Trigger-checked to reference an existing object, with the same removal order as `artwork_path`. Swapping it is a QUIET edit: only `starts_at_local` and `location` reach anyone (`20260820120000`) |
...
| `profile_emails` | profile_id FK, email (one owner per normalized address), verified_at; the addresses a member has PROVEN. The email match on `course_registrations` reads this SET plus `auth.users.email`, always as `lower(trim(...))`, through a security-definer function and never a JWT claim. **Empty as of 2026-08-11:** the self-service claim flow that filled it (`email-claim` + request/verify RPCs) was cut with ADR 0017's amendment and its objects dropped; the table and every reader stayed. **`link_registration()` is its writer as of 2026-08-31** (#164): linking a website registration by hand PROVES that address, so the member's next payment matches automatically and they stop hitting the problem they contacted the church about. It refuses outright when the address is already proven by another member, or is another account's sign-in address, because either means two people claim one mailbox, which is the mis-link this tool is most dangerous for; linking quietly without proving the address would hide that |
...
| table | key fields |
|-------|-----------|
| `books` | id, title, author, description, format enum(`pdf`\|`epub`), **price_minor int, price_currency char(3)**, **cover_path** (public-read `book-covers`), **file_path** (private `book-files`), payhip_url, payhip_product_id **unique**, payhip_product_link, **published_at null**, created_at, updated_at. **Built W4.1 slice 2** (`20260901120000`) with four corrections to this row. Both object columns are PATHS, never URLs (the W2.3/W3.1 rule: a signed URL expires and a public one pins the project host into the row). `published_at` did not exist here and had to: the dashboard creates a row, uploads two files to it and then publishes, and a book visible between those steps is a Buy button pointing at a product that is not there yet. `payhip_product_link` is the product PERMALINK, which is what Payhip's licence-verify API takes. Admins write it through RLS + grants (the W3.5 slice 5b convention), not a service-role route, and there is deliberately no DELETE: removing a title from sale is clearing `published_at`, which keeps it in the Library of everybody who bought it |
| `entitlements` | id, profile_id FK, book_id FK (**ON DELETE RESTRICT**), source enum(`payhip`\|`gift`), **source_ref text unique null** (Payhip transaction/order id, or the licence key: replays no-op), granted_at, **revoked_at null + revoked_reason null**; unique(profile_id, book_id). **A refund REVOKES rather than deletes** (W4.1 slice 2): `14` promises a re-purchase restores place and progress, and a deleted row cannot say what was taken back or why. Every read path filters `revoked_at is null`, including the `book-files` mint policy, so a refunded book leaves READER as well as LIBRARY |
| `reading_state` | profile_id (**default `auth.uid()`, outside the INSERT grant**), book_id, location text (CFI/page), updated_at; PK(profile_id, book_id). Identity is not an input (the W3.1 slice 4 rule): a client that names `profile_id` gets 42501 rather than a silent overwrite. Purged 12 months after the entitlement behind it was revoked, by `run_retention_purges()` |
...
| `payhip_events` | id, event_id text, **event_type text**, payload jsonb, received_at, processed_at null, attempts, last_error, redacted_at null; **unique(event_id, event_type)**; raw webhook inbox; processing is async and idempotent. **The pair, not `event_id` alone** (W4.1 slice 2): Payhip's refund webhook carries the ORIGINAL order id, so a single-column unique would swallow every refund as a replay of its own sale. Service-role only: FORCE RLS with ZERO policies and nothing granted to either API role, because the payload holds the buyer's email and IP |
| `unmatched_purchases` | id, buyer_email text (**normalized lowercase, CHECKed rather than assumed**), book_id FK **null** (a sale for a product not in our catalogue has no book, and the queue has to render that), source_ref text, payload jsonb, created_at, **resolved_at null + resolved_profile_id null**; drained automatically when a profile with that email later exists (the identity email is verified by sign-in, `03`); visible in the dashboard "unmatched purchases" queue (`17`). Service-role only, same posture as `payhip_events`: every row is a stranger's address |
