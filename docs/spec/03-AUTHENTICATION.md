...
- **Store-review bypass (hardened):** exactly one allowlisted review email address accepts a fixed 6-digit code, checked server-side only. (Wording updated 2026-07-26, was "long random fixed code": the code must fit `AUTH-2`'s normal 6-digit input so the reviewer types it like any member. The small keyspace is compensated: the per-environment flag below, per-IP rate limiting on the bypass endpoint, uniform denials, logging with alerts, and a fresh code per submission window.) The test account is pinned to `role='member'`; its posts are born `pending` like any member's (so the reviewer sees the honest "a leader will review this" flow, the correct demo for a moderation-first app), are **excluded from all moderation queues and can never be approved** (the moderation UPDATE policy refuses rows authored by the review account), and are purged when the flag turns off. Flag is per environment: **enabled on dev/preview** (the nightly E2E suite signs in with it, no alerting) and **disabled in production** except from submission until approval + 7 days; every production use is logged and alerts. Runbook note: Apple/Google re-review apps unprompted (post-OTA checks, complaints), so re-enabling on a re-review request is a documented one-step action. Documented in the review notes at submission (see `18`).
