...
2. **healthchecks.io** dead-man pings on every job in §5 (free tier: 20 checks). Each job reads its own check URL from a per-function secret, so a check that has not been created yet is a no-op rather than an error: `HEALTHCHECK_URL_YOUTUBE_SYNC`, `HEALTHCHECK_URL_LIVE_DETECTION`, `HEALTHCHECK_URL_MODERATION_ALERTS`, `HEALTHCHECK_URL_VERSE_MONITOR` (W2.7 slice 5), the rest as their jobs land.
