...
| dev (hosted) | EAS `preview` builds, Update channel `preview` | ~~fresh free-tier Supabase project~~ **never created** | Planned, never stood up: the daily loop is local and production is now a project of our own, so preview builds point at production (ADR 0023). Revisit when a second person needs a shared non-prod database |
| production | store builds, Update channel `production` | **our own Supabase project**, EU (`eu-central-1`), Free to start | Created at Track P Phase 1 (ADR 0023, superseding 0001). Serves the app AND the church website, which moves onto it at Phase 3. Pro before public launch; the interim rule is no sermon audio in production storage plus 80% usage alerts (`24` §1) |

**Staging:** not a permanent third environment. ~~One short-lived rehearsal project is created before the Grace Portal cleanup (see `19`)~~: there is no cleanup to rehearse (ADR 0023). The migrations apply to an empty project, which CI proves on every PR touching `supabase/`.

**Vercel (dashboard):** env vars scoped per environment. ~~Preview deployments get DEV Supabase URL/keys only~~ **there is no dev project to point them at (ADR 0023), so this needs deciding at the first dashboard deploy**: either previews get the production URL with the anon key and no service-role key at all, or preview deploys are switched off. `SUPABASE_SERVICE_ROLE_KEY` exists in the Production scope only, used exclusively in server routes, and that part does not change. Production deploys from `main`; previews per PR.
...
| Service reminders | every 15 min (W3.4 slice 1: `service-reminders`, at :01/:16/:31/:46) | computes due services per branch timezone, one hour ahead, in a window anchored to the 15-minute GRID rather than to `now()` so a late tick scans the window an on-time tick would have; `notifications.dedupe_key` (`02`) so re-runs never double-send, and the key carries the local start time so two services on one date both send | reminders silently stop (the canonical failure) |
| Event RSVP reminders | hourly (W3.4 slice 2: `rsvp-reminders`, at :11) | events starting 24 hours ahead, in a grid-anchored window; `going` only, and no pref gate (transactional, `15`); dedupe via `notifications.dedupe_key` (`rsvp_reminder:<event_id>:<starts_at_local>`, so a rescheduled event mints a new key) | promised reminders (`11`) never fire |
...
| Retention purges | monthly, 1st (W3.4 slice 3: `retention-purge`) | notifications = `purge_old_notifications()`, a batched DELETE past 12 months (ADR 0022: the table is deliberately not partitioned, because monthly partitions cannot carry either of its dedupe keys); reports > 24 months; `broadcast_deliveries` > 30 days; `payhip_events` payload redaction + 12-month purge; unclaimed `unmatched_purchases` > 12 months; `push_tickets` > 7 days; **`devices` with `last_seen_at` > 180 days** (the pruning backstop when receipts were missed: the token re-registers on next app open); batched deletes elsewhere. **Built W3.4 slice 3 as `run_retention_purges()`, covering the four tables that exist today** (notifications, `push_tickets`, `devices`, reports); `broadcast_deliveries` joins it at W3.5 and the two Payhip tables at W4.1, by extending that one function rather than adding a second job. **An OPEN report past 24 months is KEPT, counted and logged** (decided with Ayo 2026-08-19): it is a process failure rather than stale data, and deleting it would destroy the safeguarding evidence and the outstanding task in one statement. Only the notifications purge is batched, because only it can be big; the ceiling for the others is named in the migration | GDPR retention drift |
...
| Prayer reminders | hourly (W3.4 slice 2: `prayer-reminders`, at :26) | commitments whose `next_reminder_at` has come: day 1, day 3, day 7 from `committed_at`, then off the cadence (the hard cap `09` asks for). Held to 08:00-21:00 in the member's branch timezone, the only quiet-hours rule in the app, because this is the one reminder with no clock of its own. Stops on "I prayed", answered, unpublished, deleted, `prayer_reminders` off, or a block in either direction. Advances every batched row rather than every sent one, so a run that dies after sending still moves on | a member is nagged about a request that is over, or never nudged at all |
| Counter reconciliation | nightly (W3.4 slice 3: `counter-reconcile`, 02:50) | recomputes `glory_count`, `praying_count` and `prayed_count` from the reaction tables and writes back only what drifted, so a healthy night is zero updates. Writes through the `agbc.counter_write` flag, so `updated_at` is not bumped (a nightly bump would expire every leader's in-flight review token) and the family broadcast still fires, which means an open app corrects itself | drifted counts |
...
- `cron.schedule(...)` in the job's own migration runs `jobs.invoke_edge_function('<slug>')`, which reads `project_url` and `secret_key` (the `sb_secret_` key, sent as `apikey`; ADR 0024, since 2026-08-19) from **Supabase Vault** at call time and POSTs through `pg_net`. An environment whose vault is empty raises a NOTICE and does nothing, so `supabase db reset` and CI stay silent.
...
8. **Resend canary:** weekly, the verse-monitor job sends itself one email via Resend and pings a dedicated dead-man check (email verification otherwise fails silently until a buyer hits it). **Built W3.4 slice 3:** it runs on the MONDAY pass of the daily verse monitor, before that job reads the queue, so the canary reports on Resend rather than on whether the verse queue happened to be healthy. Weekly with NO ledger, deliberately: `job_alerts.recipient_id` references a profile and the canary's only recipient is itself, so keying off the weekday is stateless and the worst case is one duplicate email to our own inbox after a manual re-run. It never throws and never touches the verse monitor's own check; an environment with no Resend key pings FAILURE on a Monday, which is ADR 0016's rule read at its word.
