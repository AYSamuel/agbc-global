...
And the `media` group for the SERMON player (added W3.1 slice 3, mockup `.pl-scrub .bar` + `.pl-art`), theme-independent for the same reason `onInk` is: none of it sits on a themed surface. `track` `rgba(128,128,128,0.28)` is the scrub bar behind the gold fill, and it is a NEUTRAL grey on purpose, so the unfilled half reads on cream and on near-black alike; a themed surface token disappears into one of the two. **W4.9 slice 2 (2026-09-06): the bar is a seek control and wears a 14 px gold knob at the played edge** (`.pl-scrub .bar i::after`, same gold, a soft shadow so it reads on both themes), because a bar without a handle does not look like something you can hold; the track and fill are unchanged. `artFrom` `#22375f` → `artTo` `ink` is the artwork gradient a message with no thumbnail falls back to, `artScrim` `rgba(14,20,32,0.45)` is the wash over a real thumbnail (tuned on a device against a real photo: 58% rendered as the same navy as the fallback beside it, and 30% did not read as dimmed at all), and `tagBg` `rgba(14,20,32,0.72)` is the pill behind the "Listening" label. **W4.9 slice 4 (2026-09-06): an audio-only message's hero and row wear the LISTEN glyph**, the waveform in the same gold or white disc the play glyph sits in (`.mediahero .wavec`, `.rthumb .wave`, `MediaHero glyph="listen"`, `SermonRow` deriving it from the message), because there is nothing to play, only something to hear; it is the same waveform the player's art and the now-playing bar carry.
...
| `SegmentedControl` | scope toggle (My branch/Everywhere), theme picker, format picker, the player's Video / Audio mode, and since W4.9 slice 4 the Watch tab's Video / Audio halves under its title (the `.seg` control, 12 below the title bar) |
