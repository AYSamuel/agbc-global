...
|---|---|---|---|---|---|---|---|---|
| 1.0.2 | 24 (iOS) | `v1.0.2` | `c66e6c6` | `9db25415` | 2026-09-14 (9 min) | App Store, submitted 2026-09-15, **approved the same day (21:28 UTC, about 15 hours) and LIVE 2026-09-16** | all at once, no phased release | **The first AGBC Global build for iOS**, replacing Grace Portal 1.0.0 (18) on the existing record (Apple ID `6760579106`). Same commit and same code as the Android row below. Shipped **without verification on an Apple device**, by decision (W4.17 plan §1). Listing in English only: Apple refused the name in German (`store-submission.md` §3b). Signed with EAS-generated distribution credentials; uploaded by `eas submit` with an App Manager API key. Released by hand (the release type is MANUAL) into 174 countries. **Apple's own `itunes.apple.com/lookup` lagged the release by storefront**, still answering Grace Portal for GB, DE, NL and NG while the US had 1.0.2 and `apps.apple.com/gb/...` already read AGBC Global: read the store PAGE to know whether a release is out. The public record shows **17+** rather than 16+ on older systems, which is Apple's own mapping of a 16+ app onto a scale that has none. **What is still owed: the review bypass off by 2026-09-22** (W4.17 plan §6 step 6) |
| 1.0.2 | 24 (Android) | `v1.0.2` | `c66e6c6` | `8d6f9d40` | 2026-09-14 (57 min, 33 of them queued) | Play, **live 2026-09-14** | full (100%) | The train that carried everything merged since 1.0.1: W4.11 (tablets no longer held in portrait; R8 minification), W4.15 (share as a picture), W4.16's app (the rhythm keeps a real calendar) and #264 (Expo SDK 57 alignment, React Native 0.86.3). **Uploaded straight to production with no internal testing track** (Ayo: "we already tested locally"), which accepts that R8, the one change only a release build exercises, was first run on members' phones. Play's preview: no devices lost, **22 MB for new installs, 11.1 MB smaller than 1.0.1**, and the deobfuscation warning every earlier release carried is gone. Release notes in four languages written for 1.0.2 |
