...
|---|---|---|---|---|---|---|
| Apple Developer (church) | iOS distribution | Account Holder: Olayinka Ademiluka. Team "Amazing Grace Bible Church Global LTD", enrolled as an Organization, **Team ID `58MF8H28K5`** (a public identifier: it appears in the website's association file) | Apple Account | Ayo, **Admin since 2026-09-14** (read from Membership details) | Fee **waived**, renewal date **2027-03-10**; both developer agreements accepted 2026-09-11 | **Last shipped iOS build: 1.0.0 (18)**, not 19 as this row said (TestFlight holds 17 and 18 only; checked 2026-09-14). App Store Connect Apple ID `6760579106`. As of 2026-09-14 the team holds two Distribution certificates and one Development certificate, all created by the Account Holder, and **no keys at all, so no APNs key**; W4.17 adds EAS's own certificate, the APNs key and an App Store Connect API key |
...
| Store-review bypass (`review-signin`) | Fixed review credentials for app-store review (docs/spec/03) | Ayo | n/a (function secrets) | TBC | Free | `REVIEW_BYPASS_ENABLED` / `REVIEW_EMAIL` / `REVIEW_CODE` via `supabase secrets set` per env + local `supabase/functions/.env`. Code: a fixed 6-digit numeric code, ROTATED per window (docs/spec/03, decided 2026-07-26; compensated by the flag, per-IP rate limits, and alerts). Values in the password manager; surfaced in store review notes at W4.8. ~~NOT enabled on production~~ **ARMED ON PRODUCTION since 2026-09-04** (W4.8's Play review window, per `store-submission.md` §SUBMISSION RECORD; this row was never updated when it opened, and was found stale on 2026-09-14 by reading the secret NAMES, never their digests, since a 6-digit code's hash is reversible). Its approval + 7 deadline has passed; it stays armed through W4.17's Play 1.0.2 and first App Store reviews, and comes off at the later approval + 7 days (`docs/spec/plans/W4.17-ios-release.md` slice 8). The ADR 0023 amendment's point still stands: there is no STANDING bypass, only review windows |
| Payhip | Book sales, entitlement webhooks | Lead pastor | TBC | TBC | Payhip fees | API key handoff session planned (`24` row 13) |
| Vercel | Website + dashboard hosting | Ayo | TBC | TBC | Free | **TWO projects since 2026-08-31.** **`agbc-dashboard`** is the leader dashboard, live at **`app-dashboard.agbcglobal.com`**: imported from `AYSamuel/agbc-global` with Root Directory `apps/dashboard` (Vercel detected the pnpm workspace and resolved `@agbc/shared` with no extra configuration), Git-connected so it redeploys on every push to `main`. Its env holds ONLY public values plus the Sentry token: the project URL and the `sb_publishable_` key. **`SUPABASE_SECRET_KEY` is deliberately NOT set**, and the dashboard runs correctly without it, because `secretSupabaseKey()` is lazy and its only caller `createAdminClient()` still has zero callers; the seam exists ahead of the first moderation route that needs it. **The moment a route calls it, that variable must be added, by Ayo, since it is full database access.** Verified live: HTTPS 200, signed-out `/moderation` turned away at `/sign-in`, and a real sign-in by Ayo. **FUNCTION REGION: `fra1` (Frankfurt), pinned in `apps/dashboard/vercel.json` since 2026-09-11.** It was **`iad1` (Washington)**, read from the console that day, and nobody had chosen it: `iad1` is Vercel's documented default. Supabase is `eu-central-1`, so every auth round trip (two per page, serial) plus one to four data waves was crossing the Atlantic and back. `fra1` IS `eu-central-1`, the same AWS region as the database, not merely the same continent. It lives in `vercel.json` rather than the console deliberately (`~/.claude/standards/devops.md`: anything click-configured must be reproducible from code), and Vercel's docs say the file **overrides** the dashboard setting, so the console value is now inert. The file sits in `apps/dashboard/` because that is the project's Root Directory, which is where Vercel reads it from. **It takes effect on the next deployment, not retroactively.** agbcglobal.com. Project `agbc-website` (repo `AYSamuel/agbc-website`, local `Desktop/agbc`). **Since 2026-08-19 (Track P Phase 3) its `PUBLIC_SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` point at `agbc-production`** (the key VALUE is the `sb_secret_` key named `website`; the var name stays because the code reads it). Verified live same night: donation row, registration row, thank-you emails. The stale `agbc-web` project on Vercel is an old artifact, untouched |
...
**The new identity is ACTIVE as of 2026-09-12.** Steps 1 to 5 were run again for it, in the order below, and the ministry has two usable admins once more. Two things about that run are worth keeping.

**The migration reached `main` before it reached production, and the gap was visible in the account.** `agbc.noreply@gmail.com` was onboarded in the app while the allowlist row still existed only in git, so the bootstrap trigger had nothing to match and the profile was created as an ordinary MEMBER. Nothing had to be redone, because the migration promotes any live profile already matching an allowlist row when it applies, but the state between merge and dispatch was a real one: an address that looked onboarded and held no authority. This is `21` §3's rule with a face on it, and the tell was reading the live People record rather than the PR's green tick.

**Verify the allowlist from the database, not from this file.** The promotion landing proves the whole migration committed, since the promote block is its last statement, but that is inference. One line in the SQL editor settles it directly: `select email from public.bootstrap_admins order by email;` should return exactly `agbc.noreply@gmail.com` and `aysamuel007@gmail.com`.
...
**The tick boxes above describe the RETIRED identity and are history.** All five were done for `oami.gospel@gmail.com` on 2026-08-19; the daily admin (`aysamuel007@gmail.com`) enrolled TOTP the same night and remains at `aal2` capability.

**The same five were run for `agbc.noreply@gmail.com` on 2026-09-12**, reported complete by Ayo:
...
**Enrolment was deliberately done by Ayo alone.** A TOTP seed is the whole recovery here, since Supabase issues no recovery codes, so it must not pass through any agent session or transcript; a break-glass credential that something else has seen is not a break-glass credential.

**One precondition that is new with this address and decides whether any of it works:** sign-in is email OTP and nothing else, so that mailbox must RECEIVE mail and be read by someone. An address named "noreply" that nobody monitors is an admin identity that can never sign in, and it would fail at exactly the moment a break-glass account exists for. Raised with Ayo on 2026-09-11, confirmed as intended, and **tested on 2026-09-12 before enrolment**: mail to that address arrives and is read. Cheapest check in this section and it gates every other step, because an unmonitored mailbox is an admin identity that can never sign in.
...
- [x] ~~**By 2026-09-17:** rotate the Supabase access token `agbc-ci-deploy`~~ **IT EXPIRED FIRST, on 2026-09-17, and was found on 2026-09-18 exactly the way this entry said it would be: by a production deploy that would not run.** W4.21 needed `supabase functions deploy` and `supabase link` answered `{"message":"Unauthorized"}`; the token had already been dropped from the account's token list, so there was nothing to inspect. **The calendar reminder was the only control and it did not catch it**, which is why the replacement's date now also sits on `18`'s launch checklist, where something is read rather than remembered. **Replaced 2026-09-18, expiring 2027-09-16.** One year is Supabase's ceiling and not a preference: the picker disables every date past 2027-09-18, so a longer token cannot be asked for. That also means the expiry CANNOT be pushed far enough out to stop mattering, which is the argument for the checklist line rather than a reminder. **Scoped tokens were tried TWICE first and do not work with the pinned CLI** (see below). The original entry follows, because its reasoning is still the reasoning: **Its scope is the whole Supabase ACCOUNT, not this project**, which includes the unrelated `monietally` org, so it is the widest-blast-radius secret in the inventory. Supabase offers no non-expiring option, and the failure is silent and late: `supabase link` starts refusing weeks after anything changed, and the prod deploy is the only thing that would notice. **Worse, this token is used RARELY**, so an expiry is not discovered by routine activity; it is discovered the next time something genuinely needs shipping to production. **Decision (Ayo, 2026-08-18): keep the short expiry and put a calendar reminder on 2026-09-17**, having been offered a 90-day Custom token that would have cleared the launch run-up. The reminder is the control here; this line is only a record of it. A longer token was declined because the scope is account-wide
- **Scoped tokens (public alpha) do not work with Supabase CLI 2.110.0, which `supabase-deploy.yml` pins.** Tried twice on 2026-09-18 while replacing the expired token, because a token scoped to `agbc-production` alone would have removed the account-wide blast radius that forced the short expiry in the first place. Both attempts failed IDENTICALLY at `supabase link` with `Authorization failed for the access token and project ref pair: {"message":"Your account does not have the necessary privileges to access this endpoint"}`, first with `Project Settings: read` + `Edge Functions: read-write`, then with `Organization Projects: read` added. Two quite different permission sets producing the same error is the tell that it is not a missing checkbox: the pinned CLI predates scoped tokens and calls endpoints they cannot reach. Supabase documents no mapping from CLI command to scoped permission, so there is nothing to look up. **Worth retrying when the CLI pin moves**, because the prize is real: a project-scoped token would take this secret out of the widest-blast-radius slot in the inventory. Until then the legacy account-wide token is correct, and the thing we actually fixed is the 30-day expiry that bit us
...
- [x] ~~Complete the five activation steps again for `agbc.noreply@gmail.com`~~ **Done 2026-09-12** (identity changed 2026-09-11, `20260911120000`). Promotion verified against the live People record, not the deploy's exit code
- [x] ~~Confirm `agbc.noreply@gmail.com` can actually RECEIVE and be read~~ **Tested 2026-09-12, before enrolment.** Kept in the history rather than deleted, because it is the check that gates every other step for any future role mailbox
