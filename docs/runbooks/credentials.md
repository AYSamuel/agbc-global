...
| Store-review bypass (`review-signin`) | Fixed review credentials for app-store review (docs/spec/03) | Ayo | n/a (function secrets) | TBC | Free | `REVIEW_BYPASS_ENABLED` / `REVIEW_EMAIL` / `REVIEW_CODE` via `supabase secrets set` per env (dev/preview on; prod off outside submission windows) + local `supabase/functions/.env`. Code: 20+ random chars (the function refuses shorter). Values in the password manager; surfaced in store review notes at W4.8 |
