...
- `PLAN`: owned plans + today's day, progress ring, day list. Non-owners see the devotional's store page (`BOOK-DETAIL`) with a "Get the devotional" CTA, never an empty or locked screen.
...
- **Streak computation (specced):** an AFTER INSERT trigger on `attendance` runs an idempotent FULL recompute from `attendance` (never incremental-only, so late offline replays retro-correct), ISO weeks computed in the **home branch's timezone** (`branches.timezone`); `longest_weeks` is monotonic. A weekly lease-locked cron pass re-runs it as a safety net (`21` §5).
...
- **Visiting another branch:** "I'm here" records attendance at the BROWSED branch (see the branch-context model in `07`); streaks count attendance at any branch, computed in the home branch's timezone.
- **Stream never went live:** members who opened `LIVE` during the service window keep attendance credit, or the week auto-graces; a leader can mark "no stream today" (see `08`). Grace-framing must survive technical failures.
