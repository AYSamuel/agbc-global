...
- **Queued (idempotent one-tap actions):** attendance "I'm here", plan-day complete, RSVP status, Glory / "I prayed" toggles, playback position. Offline: enqueue locally (persisted), show optimistic UI, replay on reconnect. Safety: each of these is idempotent server-side via the unique constraints in `02`, so duplicate replays are no-ops. Conflict policy: server state wins; a rejected replay reconciles the UI quietly. The queue is small, capped (drop oldest), and cleared on sign-out.
