...
- **Staff MFA (per the security standard):** passkeys/WebAuthn on the dashboard, **mandatory for admins**, strongly recommended for leaders (a single OTP factor is phishable, unacceptable for accounts that can broadcast to the whole ministry). Step-up re-auth for role assignment, branch management, and ministry-scope broadcasts. Dedicated admin accounts; short idle timeout; every privileged action audit-logged (actor, action, target, timestamp, immutable).
