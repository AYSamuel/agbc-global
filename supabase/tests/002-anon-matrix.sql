...
-- their own, so one survivor breaks this count for every later run until somebody clears it
-- by hand. It counted raw until W3.5, when the branch module's tests started making such
-- branches; the count above it had been scoped since W2.7 for exactly this.
select is(
