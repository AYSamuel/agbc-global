...
- **The code exists and is substantial.** `apps/mobile` (Expo Router: every Phase 1-2 screen plus W3.1's player, MY-LIST and SERMON-NOTES), `apps/dashboard` (Next: moderation, people, reports, verses, sermon audio), `packages/shared`, `packages/config`, and `supabase/` (50 migrations, 35 pgTAP files, 10 edge functions, seeds, snippets). `pnpm test` runs 810 mobile + 206 dashboard tests; six Maestro E2E journeys run on the Android device (`apps/mobile/maestro/`, not in CI yet). Assume a surface is already built and go read it before planning to build it.
...
- **Phase 3 is under way. W3.1 (the audio slice) is at slice 4 of 5**, all landed: the private `sermon-audio` shelf and its dashboard upload (1a, 1b), the native rebuild carrying `expo-audio` + `expo-notifications` (2), the background/lock-screen player with server-synced resume (3), the controls redesign, and now Save + MY-LIST + SERMON-NOTES (4). **Slice 5 (message artwork, `artwork_path` + its own bucket) is the remaining piece**; the plan and every decision live in `docs/spec/plans/W3.1-audio-slice.md`, which is deleted when W3.1 lands. W3.3 owns push (and the six deferred analytics events + the two blocked Sentry DSNs land with their infrastructure, see `docs/runbooks/credentials.md`).
